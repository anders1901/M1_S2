...
{
  int my_rank, p, source, dest, localh;
  int tag = 0;
  char message[100];
  MPI_Status status;
  ...
  /*Divisibilité de la taille*/
  if (h%p != 0){
    fprintf(stderr, "Erreur nombre de processus \n");
		return 0;
  }
  localh = h / p;
  ...
  local_ima = (unsigned char *)malloc(w * localh * sizeof(unsigned char));
  ...
  printf("Dbut OPERATION ");
  y = ymin + my_rank*localh*yinc;
  for (i = 0; i < localh; i++) {
    x = xmin;
    for (j = 0; j < w; j++) {
      // printf("%d\n", xy2color( x, y, prof));
      // printf("(x,y)=(%g;%g)\t (i,j)=(%d,%d)\n", x, y, i, j);
      local_ima[j + i * w] = xy2color(x, y, prof);
      x += xinc;
    }
    y += yinc;
  }
  if (my_rank != 0) {
    MPI_Send(local_ima, strlen(local_ima) + 1, MPI_CHAR, dest, tag, MPI_COMM_WORLD);
    //sauver_rasterfile("mandel1.ras", w, h, local_ima);

  }else{
    //sauver_rasterfile("mandel0.ras", w, h, local_ima);
    strcpy(ima, local_ima);
    for (int k = 1; k < p; k++){
      MPI_Probe(MPI_ANY_SOURCE, tag, MPI_COMM_WORLD, &status);
      source = status.MPI_SOURCE;
      MPI_Recv(ima + w * localh * source, strlen(local_ima) + 1, MPI_CHAR, source, tag, MPI_COMM_WORLD, &status);
      //strcpy(ima  + 320000, local_ima);
      printf("\nRecois de %d", k);
      //sprintf(ima, "%c%c", ima, local_ima);
      //MPI_Probe(ima+local_ima, 100, MPI_CHAR, MPI_ANY_SOURCE, tag, MPI_COMM_WORLD, &status);
    }
  }
	...
	sauver_rasterfile("mandelProbe.ras", w, h, ima);
